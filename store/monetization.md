...
3. Vygenerují se **public SDK keys** (jeden pro iOS, jeden pro Android) —
   ty patří do klienta; secret keys nikam do aplikace nedávat.
...
10. V aplikaci: `Purchases.configure(apiKey:)` s platform-specific public
    key, **anonymní App User ID nechat generovat SDK** (žádné vlastní ID —
    viz privacy štítky), gating: `customerInfo.entitlements["pro"].isActive`.
...
2. **Poznámka k provizi**: u **předplatných účtuje Google 15 % od první
   ...
   „15 % na první 1 M USD/rok" se týká ostatních nákupů v aplikaci —
   pro čistě předplatitelský model Tapkasy není potřeba nic aktivovat,
...
- Párování: sekce v Menu viditelná; **Vytvořit kód** otevře paywall,
  dokud není `pro` aktivní. **Připojit se** ke kódu zůstává zdarma —
  platí jen zařízení, které kód vytváří, stánku stačí jedno předplatné
