...
`./sync-web.sh` spusť po **každé** změně `kava/index.html`, `bridge.js`
...
- `PeerLinkPlugin.swift` a `TapkasaViewController.swift` jsou přidané
  v targetu App (kopie z `ios-plugin/`; při změně uprav **obojí**, nebo po
  úpravě v `ios-plugin/` soubory znovu zkopíruj do `ios/App/App/`).
...
1. V Xcode: target **App** → Signing & Capabilities → vyber svůj tým;
   ...
   Connect / Identifiers).
2. Pro nákupy přidej capability **In-App Purchase** (StoreKit); RevenueCat
...
2. V `android/` vytvoř `keystore.properties` (necommitovat!):
...
4. V Play Console založ aplikaci, zapni **Play App Signing** (Google drží
   podpisový klíč, ty nahráváš AAB podepsaný upload klíčem) a nahraj AAB.
...
2. Založ **entitlement `pro`** a připoj k němu oba produkty
   (měsíční a roční předplatné); v nabídce (offering) použij standardní
   balíčky `$rc_monthly` a `$rc_annual`.
3. Veřejné SDK klíče vlož do `billing.js`:
...
- **Ruční úpravy Xcode projektu.** Přidání obou .swift souborů do
  ...
  dělaly editací textu, ne Xcodem. Po otevření zkontroluj, že oba soubory
